function [x,s,z,y,cond_hist,iter,success] = solve_qp_ldl(qp, verbose, atol, easy_start, max_iters, reg)
    ...
    for iter = 1:max_iters
        ...
        % get jacobian 
        K = kkt_jacobian(qp,s,z); %+ reg*speye(qp.idx.nt);
        ...
        % centering setp 
        rhs_c = rhs_centering(qp,ds_a, dz_a, sigma, mu, s);
        beta_c = K\rhs_c;
...
K(idx.x,idx.z) = G';
K(idx.x,idx.y) = A';
K(idx.s,idx.s) = diag((1 ./ s) .* z);%diag(z);
K(idx.s,idx.z) = eye(idx.ns);%diag(s);
K(idx.z,idx.x) = G;
...
function rhs = rhs_centering(qp,ds_a, dz_a, sigma, mu, s)
rhs = zeros(qp.idx.nt,1);
rhs(qp.idx.s) = (sigma*mu - (ds_a .* dz_a)) ./ s;
end
